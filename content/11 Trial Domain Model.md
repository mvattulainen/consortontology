...
| -------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `TDM-01` | Trial structure              | Every [RandomisedTrial](./domain-model/randomisedtrial) has exactly one [TrialDesign](./domain-model/trialdesign) and at least two [trial arms](./domain-model/trialarm).                                                                        |
| `TDM-02` | Arm identification           | Every [TrialArm](./domain-model/trialarm) has one `arm_identifier` and one human-readable `label`.                                                                                                                                               |
| `TDM-03` | Participant assignment       | Every enrolled [Participant](./domain-model/participant) is assigned to exactly one arm belonging to the same trial.                                                                                                                             |
| `TDM-04` | Arm intervention             | Every trial arm has at least one [Intervention](./domain-model/intervention), and every intervention has a `name`.                                                                                                                               |
| `TDM-05` | Outcome definition           | Every [OutcomeSpecification](./domain-model/outcomespecification) has an identifier, measurement variable, role, analysis metric, aggregation method, and time point.                                                                            |
| `TDM-06` | Primary outcome designation  | Every randomised trial has at least one [PrimaryOutcome](./domain-model/primaryoutcome), and each primary outcome is also one of the trial's outcome specifications.                                                                             |
| `TDM-07` | Randomisation processes      | Every randomised trial has exactly one [RandomAllocationProcess](./domain-model/randomallocationprocess) and exactly one [AllocationConcealmentProcess](./domain-model/allocationconcealmentprocess), each with its required method description. |
| `TDM-08` | Result linkage               | Every [OutcomeResult](./domain-model/outcomeresult) refers to exactly one outcome specification and has at least one [GroupResult](./domain-model/groupresult); every group result refers to exactly one arm of the same trial.                  |
| `TDM-09` | Effect and precision         | Every [EffectEstimate](./domain-model/effectestimate) has a measure type and numeric value; every attached [PrecisionEstimate](./domain-model/precisionestimate) states its interval type.                                                       |
| `TDM-10` | Participant-flow observation | Every [ParticipantFlowObservation](./domain-model/participantflowobservation) has a stage and a non-negative count.                                                                                                                              |
